...
# 导入读Oracle表、建Hive表的包
from auto_create_hive_table.cn.wwx.datatohive import CHiveTableFromOracleTable, CreateMetaCommon, CreateHiveTablePartition, LoadData2DWD
# 导入工具类：连接Oracle工具类、文件工具类、表名构建工具类
from auto_create_hive_table.cn.wwx.utils import OracleHiveUtil, FileUtil, TableNameUtil
# 导入日志工具包
from auto_create_hive_table.config import common

# 根据不同功能接口记录不同的日志
admin_logger = common.get_logger('wwx')
...
if __name__ == '__main__':
    ...
    tableList = FileUtil.readFileContent("F:\\PythonProject\\OneMake\\dw\\ods\\meta_data\\tablenames.txt")
